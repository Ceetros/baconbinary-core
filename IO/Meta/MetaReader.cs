    ...
    public class MetaReader
    {
        public (DatFile, SprFile) ReadMetaFile(string path, string key)
        {
            ...
            for (int i = 0; i < assetFileCount; i++)
            {
                ...
                for (int s = 0; s < sprites.Count; s++)
                {
                    ...
                    sprFile.Sprites[globalId] = new Sprite(globalId, ClientFeatures.Transparency) { CompressedPixels = sprites[s], Size = (ushort)(sprites[s]?.Length ?? 0) };
                }
            }
            
            sprFile.SpriteCount = (uint)sprFile.Sprites.Count;

            bodyStream.Seek((itemCount + outfitCount + effectCount + missileCount) * 4, SeekOrigin.Current);
        ...
        private void ReadThings(BinaryReader reader, uint count, ThingCategory category, uint startId, Dictionary<uint, ThingType> dictionary)
        {
            for (uint i = 0; i < count; i++)
            {
                ...
                if (reader.BaseStream.Position != startPos + blockSize)
                {
                    reader.BaseStream.Position = startPos + blockSize;
                }
            }
        }
    }
